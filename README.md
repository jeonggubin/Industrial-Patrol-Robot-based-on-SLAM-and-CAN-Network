...
> ⚠️ **[이슈] CAN ID 중복 할당**
> 현재 설계 상 라즈베리파이가 STM32F446으로 보내는 모터 주행 명령 ID가 `0x100`으로 할당되어 있으나, 센서 보드인 STM32F429ZI의 헤더(`can.h`)에도 서보 제어 수신 ID가 `0x100`으로 중복 정의되어 있습니다. 프로그램 구동 시 F429 보드에도 주행 패킷이 수신될 수 있으므로, 향후 F429의 프로토콜 ID를 다른 대역으로 완전히 분리하는 작업이 필요합니다.
...
*   **안전 정지:** 주행 방향(전진/후진)과 무관하게, 초음파 거리 측정값이 **15cm 이하로 2회 연속** 잡혀 긴급 제동(SAFE_ESTOP) 상태에 진입하면 즉각적으로 모터 출력을 0으로 강제 차단합니다 [1, 2].
*   **정지 해제:** 측정 거리가 **20cm 이상 2회 연속** 잡힐 경우 다시 정상 주행이 가능하도록 복귀됩니다. (센서 타임아웃 200ms 초과 시에도 강제 정지 처리) [2]
...
### 5. 센서 수집 및 시스템 관제 (STM32F429ZI)
STM32F429ZI 보드는 방대한 GPIO 확장성을 바탕으로 시스템의 '센서 허브' 역할을 수행합니다. 다량의 작업장 환경 데이터를 병렬로 안전하게 수집하고, 시스템 경보 및 2대 보드 간 CAN 통신망을 총괄 제어합니다.

*   **하드웨어 분리 설계:** 모터 구동 시 발생하는 큰 피크 전류와 전원 노이즈가 정밀 센서 신호에 왜곡을 주지 않도록 시스템을 446(주행)과 429(센서)로 물리적 이원화하여 강인성을 확보했습니다.
*   **가청 대역 기반 경보 및 상태 출력:** 산업 현장의 소음을 극복할 수 있는 경고 시스템을 내장하고 있습니다.

| 항목 | 하드웨어 설정 값 / 제어 기준 | 비고 |
| :--- | :--- | :--- |
| **조도 샘플링 시간** | **84 사이클 (Polling Mode)** | 12비트 ADC 충전 시간 충족 (메인 루프에서 Polling 방식으로 수집) |
| **RGB LED 주파수** | **1 kHz** (PSC: 179, ARR: 999) | 사람 눈의 깜빡임 방지, Auto-reload preload 활성화로 Glitch 차단 |
| **경고 부저 주파수** | **500 Hz** (PSC: 167, ARR: 999) | APB1 타이머 클럭(84MHz) 기준 500Hz 출력 설정 (50% Duty Cycle) |
| **소모 전류 (Peak)** | 약 **200mA ~ 280mA** (서보모터 제외) | 피크 800mA인 서보모터를 446으로 분리하여 429 보드 전압 강하 방지 |

###### 💡 운영 모드 및 시스템 연동 로직
*   **수동 모드 (Manual Mode):** 원격 관제 센터의 관리자가 정밀 점검이나 특정 이미지 캡처를 수행할 때 사용자 명령대로 LED 색상과 On/Off 상태를 제어하여 운영의 유연성을 확보합니다.
*   **CAN 관제 인터페이스:** 500kbps 표준망을 통해 유입되는 긴급 명령에 따라 경고 LED와 부저를 즉각 제어하며, 수집된 모든 센서의 로우 데이터를 실시간으로 역전송하여 관제 대시보드와 동기화합니다.

> ⚠️ **[이슈] 조명 자동 제어(Auto Mode) 로직 비활성화**
> 초기 설계에는 주변 조도에 따라 RGB LED가 스스로 점등되는 'Auto Mode'와 조도 센서 연속 변환 기능이 기획되었습니다. 하지만 최종 통합 테스트 과정에서 수동 관제 명령과의 충돌 방지를 위해 현재 펌웨어(`main.c`)에서는 해당 로직을 비활성화해 두었습니다. 따라서 현재 조도 센서는 Polling 방식으로 측정되며, 조명 제어는 웹 대시보드를 통한 수동 원격 제어만 지원합니다.
...
## 🎬 시연 영상
[![SLAM 및 CAN 네트워크 기반 산업용 순찰 로봇](https://i.ytimg.com/vi/PNa3HpqTolg/maxresdefault.jpg)](https://youtu.be/PNa3HpqTolg?si=JT_z5yxzs2ES-m6S)
