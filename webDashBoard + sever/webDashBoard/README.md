# 📊 Robot Patrol Web Dashboard

로봇 순찰 시스템의 실시간 상태 확인과 원격 제어를 위한 웹 대시보드입니다.

브라우저에서 실행되며 Socket.IO를 통해 서버와 통신하고, JSMpeg WebSocket 스트림을 통해 카메라 영상과 맵 영상을 표시합니다.
...
| `style.css` | UI 디자인 및 반응형 레이아웃 |
| `script.js` | Socket.IO 통신, 영상 스트림 연결, 버튼 제어 로직 |
...
대시보드는 현재 접속한 호스트 주소를 기준으로 서버에 연결합니다.
...
자율 주행 모드 ON 시 다음 명령을 서버로 전송합니다.
...
또한 모터 속도, 초음파 거리, 조도, 카메라 각도, 경고 상태, 서버 연결 상태를 실시간으로 확인할 수 있습니다.
